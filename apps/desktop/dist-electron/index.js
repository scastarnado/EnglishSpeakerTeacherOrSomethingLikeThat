  ...
  return wrapAsync(async function dispatchHttpRequest(resolve, reject, onDone) {
    ...
    if (socketPath) {
      ...
      if (allowedSocketPaths != null) {
        ...
        const resolvedSocket = path$1.resolve(socketPath);
...
function registerIPCHandlers(services) {
  ...
  electron.ipcMain.handle("ai:list-image-models", async () => {
    const client = aiService.getClient();
    if (!client) {
      return [];
    }
  ...
  electron.ipcMain.handle("system:select-webui-launcher", async () => {
...
class AIServiceManager {
  constructor(userDataPath) {
    ...
    if (process.env.NODE_ENV === "development" || !electron.app.isPackaged) {
      ...
      const appsDir = path$1.dirname(appPath);
      this.servicePath = path$1.join(appsDir, "ai-service");
      console.log("[AIService] Calculated service path:", this.servicePath);
    } else {
      this.servicePath = path$1.join(process.resourcesPath, "ai-service");
    }
  }
  async start() {
    var _a, _b;
    try {
      ...
      const bundledWebuiPath = path$1.join(
        process.resourcesPath,
        "local-ai",
        "stable-diffusion-webui",
        "webui-user.bat"
      );
      const hasBundledWebui = electron.app.isPackaged && fs__namespace.existsSync(bundledWebuiPath);
  ...
  async cleanupOldRecordings(retentionDays) {
    ...
    try {
      ...
      for (const dir of sessionDirs) {
        const dirPath = path$1.join(this.audioDir, dir);
        const stat = await fs__namespace$1.stat(dirPath);
...
class DatabaseManager {
  constructor(userDataPath) {
    ...
    const dataDir = path$1.join(userDataPath, "data");
    this.dbPath = path$1.join(dataDir, "app.db");
  }
  async initialize() {
    try {
      await fs__namespace$1.mkdir(path$1.dirname(this.dbPath), { recursive: true });
      const isDev = !electron.app.isPackaged;
      let wasmPath;
      if (isDev) {
        const workspaceRoot = path$1.join(electron.app.getAppPath(), "../..");
        wasmPath = path$1.join(
          workspaceRoot,
          "node_modules/sql.js/dist/sql-wasm.wasm"
        );
      } else {
        wasmPath = path$1.join(process.resourcesPath, "sql-wasm.wasm");
      }
...
async function initializeServices() {
  ...
  try {
    try {
      databaseManager = new DatabaseManager(electron.app.getPath("userData"));
    ...
    try {
      aiServiceManager = new AIServiceManager(electron.app.getPath("userData"));
